...
impl Cors {
    pub fn new(allowed_origins: Vec<String>, allowed_methods: Vec<String>, allowed_headers: Vec<String>, allow_credentials: bool) -> Self {
        Self {
            allow_all: false,
            allow_credentials,
            allowed_origins,
    ...
    async fn after(&self, data: &mut ServiceData) -> WrapperResult {
        ...
        if self.allow_all {
            ...
            if let Some(origin) =  headers.get("origin") {
                if self.allowed_origins.contains(&origin.to_str().unwrap_or_default().to_string()) {
                    data.response.headers_mut().insert(
                        HeaderName::from_static("access-control-allow-origin"),
                        origin.clone(),
                    );
