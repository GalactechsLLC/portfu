...
use uuid::Uuid;
use prometheus::{self, Registry, HistogramOpts, HistogramVec, TextEncoder};
...
static REQUEST_SIZES: Lazy<Arc<HistogramVec>> = Lazy::new(|| {
    Arc::new(
        HistogramVec::new(
            HistogramOpts::new("request_sizes_histogram", "Request Sizes")
                .buckets(vec![
                    0f64,
                    1024f64,
                    16f64 * 1024f64,
...
impl ServiceHandler for MetricsEndpoint {
    ...
    async fn handle(&self, mut data: ServiceData) -> Result<pfcore::ServiceData, (ServiceData, Error)> {
...
impl Default for MetricsWrapper {
    fn default() -> Self {
        Self {}
    }
}
...
impl WrapperFn for MetricsWrapper {
    ...
    async fn before(&self, data: &mut ServiceData) -> WrapperResult {
        ...
        match &new_body {
            BodyType::Stream(_) => {
                REQUEST_SIZES.with_label_values(&[
    ...
    async fn after(&self, data: &mut ServiceData) -> WrapperResult {
        ...
        match &new_body {
            BodyType::Stream(_) => {
                RESPONSE_SIZES.with_label_values(&[
                    data.request.request.method().as_str(),
                    data.request.request.uri().path(),
                    data.response.status().as_str()
                ]).observe(-1f64)
        ...
        if let Some(tracking_struct) = data.request.remove::<TrackingStruct>() {
            RESPONSE_TIMES.with_label_values(&[
                data.request.request.method().as_str(),
                data.request.request.uri().path(),
                data.response.status().as_str()
            ]).observe(Instant::now().duration_since(tracking_struct.timer).as_secs_f64());
        }
        WrapperResult::Continue
    }
}
...
async fn convert_to_fixed(body: BodyType) -> BodyType{
    match body {
        BodyType::Stream(s) => {
            match s.collect().await {
                Ok(body) => {
                    BodyType::Sized(Full::from(body.to_bytes()))
                }
                Err(e) => {
