...
impl Task {
    pub(crate) fn render(&self, output: &mut TokenStream2, interval_ms: Option<u64>) {
        ...
        for (ident_val, ident_type, inner_type) in &self.state_args {
            dyn_vars.push(quote! {
                let #ident_val: #ident_type = merged_state
                    ...
                    .ok_or_else(|| {
                        ::portfu::prelude::PortfuError::Parsing(
        ...
        let stream = quote! {
            ...
            fn #register_name(
                server: ::std::sync::Arc<::portfu::prelude::Server>
            ) -> ::std::pin::Pin<Box<dyn ::std::future::Future<Output = Result<(), ::portfu::prelude::PortfuError>> + Send + 'static>> {
                Box::pin(async move {
                    let merged_state = {
            ...
            ::portfu::prelude::inventory::submit! {
                ::portfu::prelude::TaskRegistration {
                    run: #register_name
                }
            }
        };
