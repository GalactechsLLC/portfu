            ...
            for segment in segments.iter().filter_map(|v| match v {
                ...
                variables.push(
                    quote! {
                            ...
                            impl ::portfu::prelude::PathName for #segment {
                                const NAME: &'static str = #seg_lit;
                            }
                            let #segment: ::portfu::prelude::PathImpl::<#segment> = ::portfu::prelude::FromRequest::try_from(request).await?;
                            let #segment: ::portfu::prelude::Path = #segment.into_path();
...
pub(crate) fn extract_method_filters(methods: &HashSet<Method>) -> TokenStream2 {
    ...
    if methods.len() > 1 {
        let other_method_guards: Vec<TokenStream2> = others
            .map(|method| {
                quote! {
                    ::portfu::prelude::filters::method::#method.clone()
                }
            })
            .collect();
        quote! {
            .filter(
                ::std::sync::Arc::new(::portfu::prelude::filters::any(
                    String::new(),
                    &[
                        ::portfu::prelude::filters::method::#first.clone(),
                        #(#other_method_guards),*
                    ]
                ))
            )
        ...
        quote! {
            .filter(::portfu::prelude::filters::method::#first.clone())
        }
    }
}
