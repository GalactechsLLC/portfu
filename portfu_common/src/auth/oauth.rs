...
async fn get_json(url: impl AsRef<str>, token: &OAuthToken) -> Result<Value, PortfuError> {
    reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| PortfuError::Internal(format!("Failed to build OAuth HTTP client: {e}")))?
        .get(url.as_ref())
...
impl OAuthClient {
    pub fn new(config: OAuthConfig) -> Result<Self, PortfuError> {
        let oauth_client = BasicClient::new(ClientId::new(config.client_id))
            ...
            .set_auth_uri(AuthUrl::new(config.auth_url).map_err(|e| {
                PortfuError::Parsing(format!("Invalid oauth auth_url configuration: {e}"))
            })?)
            .set_token_uri(TokenUrl::new(config.token_url).map_err(|e| {
        ...
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                PortfuError::Internal(format!("Failed to build OAuth HTTP client: {e}"))
            })?;
        Ok(Self {
            oauth_client,
            http_client,
    ...
    pub async fn exchange_code(
        ...
        let oauth_http_client = move |request| {
            let http_client = http_client.clone();
            async move { execute_oauth_request(&http_client, request).await }
        };
        let token = self
            .oauth_client
...
async fn execute_oauth_request(
    ...
    let response = client
        .execute(request.try_into().map_err(OAuthHttpError::Request)?)
        .await
    ...
    builder
        ...
        .map_err(OAuthHttpError::Response)
}
